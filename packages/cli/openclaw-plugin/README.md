...
| -------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `stroqBin`     | `stroq.json`, else `stroq` on PATH | Path to the Stroq binary to spawn, as one string (never split on spaces). `STROQ_BIN` is read before `stroq.json`. A launch command needing extra arguments belongs in `stroq.json`'s `command` array instead.                                                                                                               |
| `workspace`    | the Gateway's `process.cwd()`      | The project directory for the secret index and path rules. A tool call's own `cwd` is never used for this: honouring one would let any tool point it at an empty directory and slip past a secret-egress guard — `exec`'s shell directory is unaffected, since the CLI adapter reads it from the call's own params directly. |

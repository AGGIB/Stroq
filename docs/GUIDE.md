...
- [Which of your credentials already reached a model provider](#which-of-your-credentials-already-reached-a-model-provider)
...
6. `stroq attack` replays twenty-one recorded incidents and synthetic cells against the same policy: 15 blocked, 6 asked, 0 passed through.
...
- **Twenty-one scenarios you can replay.** `stroq attack` runs recorded hook events from public incidents and synthetic matrix cells through your own policy and reports `blocked` / `asked` / `passed` per scenario, with the source of each. It is how we check that a change to the classifier or the default policy does not silently let an old attack back in.
...
- **Taint-aware policy.** The decision about an action knows whether the agent has read something suspicious in this session. Eighteen action classes, one ordered YAML policy, first match wins.

...
- Task 3 (review): CRITICAL — `mcpCallInput`'s brief text unconditionally overwrote a colliding `inputResponses` key with the top-level retry field, so a secret placed under `arguments.inputResponses` (a hostile server can name a tool parameter exactly that and tell the model to put a credential there) was silently dropped before the guard ever ran, and the call was forwarded (`forward: true`) instead of denied. Fixed to keep both values under distinct keys: when `arguments` already has `inputResponses`, the top-level field is written under the next free `inputResponses_…` key instead of overwriting.
- Task 3 (review): `withWarningBlock` discarded a non-array `content` value entirely instead of preserving it as the first item ahead of the appended warning; a malformed or legacy result's existing content is data, not noise.
- Task 3 (review): `refuseBatch` audited every `tools/call` it denied except an idless one, which produced no reply (by design, per the spec) but also left no audit trail. An idless `tools/call` inside a refused batch is now audited directly through `denyDirectly` (no reply pushed, since none can be sent), so `stroq log` explains every `tools/call` a refused batch carried, not only the ones a reply could be sent for.
- Task 3 (review): `tools/list` scanning covered `name`/`title`/`description`/`annotations` but not `inputSchema`, whose parameter `description`s are exactly as model-visible a rug-pull surface as the tool's own description. `inputSchema` is now included as JSON text; spec §2a's `tools/list` sentence amended to match.
- Task 3 (review): the post-scan path passed `toolInput: {}` to the engine for every scanned MCP result, so every post audit line's summary read the unreadable literal `"{}"`. It now passes `{ method: <the pending request's method> }` (no argument values, so nothing secret-shaped), so the summary reads e.g. `{"method":"tools/list"}`.
- Task 3 (review): no code change — `mcpMethodToolName('<server>', 'tools/call')` (the `MCP_METHOD_TOOL['tools/call']` entry) was implemented but untested; a shape test now covers it.
- Task 3 (review): no change in this branch — `askAsDeny` is a third near-copy of the Codex/Windsurf ask-as-deny renderers. Extracting it into `pre-decision.ts` is a recorded follow-up, deferred because the shared adapter modules are frozen for the remainder of this branch.
- Task 3 (review fix round 1): applying the above pushed `judge.ts` past the repo's 400-line file cap. The result-text extraction half (`MCP_MAX_RESULT_CHARS`, `mcpResultText`, `resultTextFor` and their private helpers) moved into a new `packages/cli/src/mcp/mcp-result-text.ts`; `judge.ts` re-exports the same three names unchanged, so Task 4's `from '../mcp/judge.js'` imports are unaffected and the module's public surface is identical to what the brief specified.
- Task 4 (review): CRITICAL — neither direction's `data` handler honoured backpressure: `options.stdin`/the server's stdout were drained unconditionally regardless of whether the corresponding forward/reply had drained, so a fast writer paired with a slow reader grew the per-direction queue without bound (measured: 100k lines from a fast server against a slow-reading client pushed RSS past 380 MiB, with the server never slowed by anything the proxy did). Fixed by pausing the SOURCE stream whenever a write to that direction's destination is outstanding (`write()` returned false and `drain` has not fired) and resuming it once the write settles (`drain`, `error` or `close` alike) — in both directions, whichever physical stream the write actually targets (a client-triggered reply written to `stdout` still pauses `stdin`, since it is client input that must slow down). The mechanism (`writeBackpressured`) is exported from the new `proxy-pump.ts` for direct unit testing.
- Task 4 (review): a relayed SIGINT/SIGTERM only forwarded the signal and waited indefinitely; a server that ignored it left both processes alive forever. Armed the same SIGKILL escalation the EOF shutdown path already used, after the same grace period. `McpProxyOptions` gained `shutdownGraceMs?: number` (default `SHUTDOWN_GRACE_MS`) so both the EOF and signal-escalation paths are exercisable with a short grace in tests, and `packages/cli/test/mcp/ignore-sigterm-server.mjs` (a stub that traps and ignores both signals) was added so a test can actually distinguish "the server ignored the signal" from "the server exited on it anyway."
- Task 4 (review): one `logError` call per oversize SEGMENT, not per oversize LINE — a 10 MiB server line arriving as dozens of streamed chunks produced dozens of synchronous log appends, each carrying a full stack trace. Fixed to log once on a run's first segment and reset only when a segment with `eol === '\n'` closes the run — keyed on `eol`, never on a segment's `text` being empty, since a legitimate mid-run chunk boundary can produce exactly that.
- Task 4 (review): CRITICAL — a BOM-prefixed `tools/call` line failed `JSON.parse` outright (some servers, e.g. Jackson/.NET stacks, are documented to reject a BOM themselves, and a client paired with one may forward it as received) and was forwarded UNCHECKED, including any secret in its arguments; separately, a request whose `method` was cased differently from the literal string `"tools/call"` (e.g. `"Tools/Call"`) fell through to the generic "forward as an ordinary request" branch, bypassing judgment entirely. Fixed three ways, applied in order: (a) a leading BOM is stripped before `parseLine`, with the ORIGINAL bytes (BOM included) still what a forward sends on allow; (b) `tools/call` is matched via `method.toLowerCase() === 'tools/call'` in both the request and the unaddressable-id notification branches; (c) a client line that STILL fails to parse after the BOM strip, and whose raw text matches `/tools\/call/i`, is denied fail-closed under a new local decision (`mcp-proxy-unparseable-call`) and dropped rather than forwarded blind — every other unparseable client line is still forwarded unchanged, as before. Spec §2a's "a line that is not JSON" sentence amended to state all three.
- Task 4 (review): several corrections from Task 4 and this review round had no test exercising `runMcpProxy`/the pump directly. Added: a non-JSON client line forwarded unchanged without stalling the queue; `notifications/cancelled` dropping a pending id so its eventual response is forwarded unscanned with no `post` audit entry; the fail-closed reply AND audit entry when `engine.pre` throws while judging a `tools/call` (an injected broken session store, mirroring `judge-decisions.test.ts`'s `brokenEngine()`); a server command that cannot be spawned at all exiting 1 with a message on stderr well under the shutdown grace; the oversize-line and signal-escalation corrections above each got their own dedicated test.
- Task 4 (review, minor): stopped reusing judge.ts's `MCP_MALFORMED_CALL` for the unusable-id branch — its reason names a missing/invalid tool NAME, which is false there (the name may be perfectly fine; it is the id that makes no reply possible). Defined a local `mcp-proxy-unaddressable-call` decision in `proxy-pump.ts` with a reason that actually matches the defect; the audit summary text is unchanged.
- Task 4 (review, minor): the internal-error deny (engine.pre throwing while judging a `tools/call`) was `logError`'d but never audited, so `stroq log`/`why` could not explain it. Now also audited through `denyDirectly` under a local `mcp-proxy-internal-error` decision whose reason is the same fail-closed text already shown to the client (which names no argument values).
- Task 4 (review, minor): `proxy.e2e.test.ts` had no guarantee a failed assertion mid-test wouldn't leave a proxy or fake-server child running past that test. Added a module-level `tracked()`/`afterEach` that SIGKILLs any child still alive when a test ends, wired into both places this file calls `spawn`.
- Task 4 (review, minor): `commands/mcp.ts`'s `parseMcpArgv` accepted the literal string `"--"` as an option's value but not another flag spelling (e.g. `--server --client x` silently set `server` to `"--client"`). Broadened the check to any value starting with `--`.
- Task 4 (review, minor): `commands/mcp.ts`'s `--cwd` was used as given, so a relative value was resolved by whatever the OS happened to interpret it against inside `runMcpProxy`/the engine, not necessarily the proxy's own directory. Extracted `resolveMcpCwd` (`path.resolve(cwd ?? process.cwd())`) so a relative `--cwd` is always made absolute against the proxy's own `process.cwd()` before use, and so the resolution is unit-testable without spawning the long-running proxy itself.
- Task 4 (review, minor): the final stdin cleanup used `removeAllListeners('data')`, which would remove a caller's OWN `data` listener on the same stream, not just the proxy's. Changed to `off('data', onStdinData)`, removing only the one listener this function registered.
- Task 4 (review fix round 1): applying the above pushed `proxy.ts` well past the repo's 400-line file cap. Split into `proxy.ts` (spawn, signal handling, shutdown lifecycle — the brief's own suggested seam) and a new `packages/cli/src/mcp/proxy-pump.ts` (the two per-line handlers, `OrderedQueue`, `writeBackpressured`, and the proxy's own local `Decision` constants). `runMcpProxy`'s public options/behaviour are unchanged; `createPump` is an internal seam Tasks 5–6 do not need to know about.
- Task 4 (review, round 2): IMPORTANT — the proxy never exited while a client kept writing to a server that had already died. `writeBackpressured` wrote unconditionally, and a destroyed sink (a `ChildProcess`'s stdin the moment the child exits: `destroyed`/`closed` true, `writable` false) returns `false` from `write()` and then emits NOTHING — `drain`, `error` and `close` all fired once, as the stream died, and an `EventEmitter` never replays a past event to a listener attached afterwards — so the helper awaited forever, left the source paused and stalled `await toServer.idle()` in `runMcpProxy` (reviewer's probe: no exit within 20 s, against 131 ms with nothing written after the death). Fixed with a `isDeadSink` check BEFORE the write (a dead sink is never written to) and again the moment a write reports backpressure (that write can itself be what observes the death); on either path the line is dropped — there is no destination left — and the source is resumed so no earlier pause outlives the sink. `writeBackpressured`'s doc comment corrected to state this.
- Task 4 (review, round 2, low): `runMcpProxy`'s EOF handler arms `termTimer`/`killTimer` inside a QUEUED `toServer` task, so a client EOF landing while the proxy was still draining could arm a SIGTERM/SIGKILL pair AFTER shutdown had cleared them — a kill nothing would ever cancel, holding the event loop open for twice the grace period aimed at a server already gone. Guarded with a `shuttingDown` flag set when the child closes (never arm once shutdown has begun), and both timers are cleared a second time after `await toServer.idle()`.
- Task 4 (review, round 2, low): `notifications/cancelled` was matched case-sensitively while `tools/call` had already been case-folded in round 1, so a `Notifications/Cancelled` left the pending id in the table and its response was scanned and audited as if never cancelled. Now matched via `method.toLowerCase()`, the same way `tools/call` is.
- Task 5 (review): CRITICAL — a PRESENT-but-non-object `mcpServers` (an array left by a hand edit, say) was replaced with `{}` by `wrapMcpConfig`/`unwrapMcpConfig` and written to disk with exit 0 and an "installed" line, destroying every server the file held; `--unwrap` did the same. Fixed at two layers: `rewrite()` (shared by both functions) now returns the config UNCHANGED whenever `mcpServers` is present and not a plain object, rather than coercing it to `{}`; and `initMcp` (now in `init-mcp.ts`) checks the new `hasValidMcpServers` export before ever calling either function, printing `cannot rewrite <file>: mcpServers is not an object` on stderr and returning 1 — before wrapping, unwrapping, or `--dry-run`'s preview — so the file is never even re-serialised. Tests cover both wrap and unwrap at the unit level (config returned byte-for-byte equal) and the full `init` path at the integration level (file bytes unchanged on disk, both with and without `--dry-run`).
- Task 5 (review): CRITICAL — non-string `args` elements (e.g. a port number) were silently dropped by a `.filter(... typeof arg === 'string')`, so `{"command":"node","args":["s.js","--port",8080]}` wrapped into a dangling `--port` with its value gone, and `--unwrap` could never restore the original; a non-array `args` (e.g. a hand-written string) was read as empty, discarding it outright on wrap. Fixed by coercing every element with `String(arg)` — matching what `child_process.spawn` does with a non-string argv entry anyway — instead of dropping it, in both `unwrapArgs`'s tail extraction and a fresh wrap's args; and by refusing to touch (not wrap, not unwrap) any entry whose `args` is present but not an array, with a new `skipped (args is not an array)` outcome. Tests cover numeric/boolean coercion surviving a wrap and an unwrap round trip, and the new skip outcome for both wrap and unwrap.
- Task 5 (review): IMPORTANT — `wrapperIndex` recognised any entry with an `index.js`-shaped path followed by `mcp --server <name>` as Stroq's own wrapper, so a foreign server whose own command happened to be e.g. `node ./dist/index.js mcp --server foo -- x` was misread as "already wrapped" and rewritten destructively instead of wrapped fresh. `init` always writes `--server <name> --client <client>` in that exact order, so recognition now additionally requires `args[i+3] === '--client'`. A test exercises the foreign-entry case directly (`wrapperIndex` returns null) and through `wrapMcpConfig` (reported as `wrapped`, with the foreign command preserved verbatim behind the new wrapper's own `--`; a second wrap of the result is then correctly recognised as `already wrapped`).
- Task 5 (review): IMPORTANT — `existsSync` accepts a directory, so `--config <dir>` (or `--config ""`, which `resolve('')` collapses to `process.cwd()` — also a directory) reached `readFileSync` and threw an uncaught EISDIR instead of the friendly "no MCP config" message. `init-mcp.ts` now guards with `existsSync(path) && statSync(path).isFile()`; tests cover both a directory and an empty `--config`.
- Task 5 (review): IMPORTANT — `init.ts` had grown to 462 lines, over the repo's 400-line file cap, by carrying the whole MCP installer inline; it also called `hookCommand(node, entry, agent as HookAgent)` with an unsafe cast even when `agent === 'mcp'` (a value `hookCommand` was never meant to receive), computing and discarding the result. `MCP_NOTE`, `mcpTarget`/`McpTarget`, `McpOptions` and `initMcp` moved to a new `packages/cli/src/commands/init-mcp.ts` (130 lines); `init.ts` is back to 374. `runInit` now checks and delegates the `agent === 'mcp'` branch BEFORE computing `hookCommand`'s `command`, and validates `agent` through a real type predicate (`isInitAgent`, exported alongside `INIT_AGENTS`/`InitAgent`) rather than an `as InitAgent` cast — after the `'mcp'` branch returns, TypeScript narrows `agent` to a plain `HookAgent` on its own, so `hookCommand(node, entry, agent)` and `install[agent](...)` need no cast at all.
- Task 5 (review, minor): the per-entry output line put the skip qualifier before the name (`skipped (http) remote`) rather than after it (`skipped remote (http)`, the spec's own order). `init-mcp.ts` gained a small `formatOutcome` that splices a trailing `(qualifier)` back in after the name; the pinned "wraps every stdio server…" test in `init.test.ts` was updated to the new order.
- Task 5 (review, minor): under `--dry-run`, the per-entry outcome lines were skipped entirely — only the JSON preview printed. They now print unconditionally, but to stderr instead of stdout while `--dry-run` is set (the same split the Copilot installer's replacement notice already uses), so stdout stays pure JSON for a `--dry-run | jq` pipeline; a test asserts `JSON.parse` on dry-run stdout does not throw and that stderr carries the outcome line.
- Task 5 (review, minor): `countWrapped` counted any recognisably-wrapped entry as `wrapped`, even one whose recorded entry file no longer exists on disk (an upgrade or uninstall that removed the old path without re-running `init`) — such a server would fail to start, yet `doctor` reported it as protected. `McpProxyCount` gained a `stale` field; a wrapper counts toward `wrapped` only when `existsSync` confirms its recorded entry path, otherwise toward `stale`. `doctor`'s detail line now appends `(N stale wrapper(s): entry missing)` when `stale > 0`. Fixture note: this made every pre-existing test that wrapped with a deliberately-fake `entryArgv` (e.g. `/x/dist/index.js`, chosen because no other check cared whether it existed) register as stale; the `countWrapped`- and `doctor`-facing tests were updated to wrap through a real, empty, temp `index.js` file instead.
- Task 5 (review, minor): `init.test.ts` had no `beforeEach` pinning `HOME`/`STROQ_HOME`, unlike `doctor.test.ts` — harmless for the cases the brief wrote (all project-scoped or `--config`-pointed), but a latent risk for any future `--client claude-desktop`/`windsurf`/`cursor --user` case. Added the same pattern `doctor.test.ts` uses, at module scope so it covers the whole file.
- Task 5 (review, minor): `initMcp`'s `if (node === undefined) return 1;` (an argv the caller can never actually produce, since `runInit` always supplies `hookArgv(process.execPath, entry)`) failed silently with no message. Now writes `stroq init --agent mcp: no node executable available to wrap with` to stderr first.
- Task 5 (review, minor): `claudeDesktopPath` read `platform()` and `process.env` internally, so its three branches (macOS/Windows/Linux) could only be exercised by mocking globals. Now `claudeDesktopPath(plat = process.platform, env = process.env, home = homedir())`, exported and called with all three explicit at its one call site in `mcpConfigPath`; a direct test covers all three branches plus the APPDATA-unset Windows fallback.
- Task 5 (review, for the record — no change): the entry-basename half of `wrapperIndex`'s recognition (`STROQ_ENTRY`) hardcodes `index.{js,ts,mjs,cjs}` and `stroq(.js|.cmd)?`; a future rename of the shipped entry file would make this check miss Stroq's own wrapper and wrap it a second time rather than replacing it. Recorded as a follow-up, not fixed in this branch.
- Task 6 (review): IMPORTANT — `run-mcp-demo.sh` never exercised `stroq init --agent mcp --config <file> --dry-run`, the installer's other entry point besides the five proxy scenarios it already drove. Added scenario 6: a temp `mcpServers` file with one stdio entry and one HTTP entry, run through the built CLI's `--config <file> --dry-run` from the demo project directory; asserts stderr carries `wrapped widgets` and `skipped remote-widgets (http)`, the config file on disk is byte-identical afterwards, and — via a small structural Node check rather than a grep — the wrapped entry's `command` equals `process.execPath` and its `args` equals `[<entry>, 'mcp', '--server', 'widgets', '--client', <config basename>, '--cwd', <project>, '--', 'npx', '-y', 'widgets-mcp-server']`, with the HTTP entry left byte-for-byte unchanged. Building the assertion surfaced a portability wrinkle worth recording: computing the expected `--cwd` from the bash variable `$demo_cwd` failed on macOS, because `mktemp -d` there returns a path through `/var` (a symlink to `/private/var`) while `process.cwd()` — what `init` actually records — reports the resolved, symlink-free form; `pwd -P` inside the same directory is what the check now compares against instead.
- Task 6 (review): IMPORTANT — README.md's example `deny-secret-egress` output block used invented prose (`Evidence: DEMO_API_KEY from .env`) rather than the real sentence `describeSecretHit` (`packages/core/src/secrets/describe.ts`) produces (`the arguments contain the value of <name> from <source>.`), which the demo's own captured output confirmed word for word. Corrected to the real wording, with the path abbreviated (`/Users/me/project/.env`, consistent with the section's other example) but nothing else changed.
- Task 6 (review): IMPORTANT — CHANGELOG.md's enumeration of `stroq init --agent mcp`'s per-entry outcomes omitted `not wrapped <name>` (the eighth `McpEntryAction` member, emitted by `unwrapEntry` when `--unwrap` targets an entry that was never wrapped). Added to the list. README.md and packages/cli/README.md do not enumerate the full outcome set anywhere (only individual examples in prose), so neither needed the same fix.
- Task 6 (review): IMPORTANT — CHANGELOG.md and the spec's §2c both described `doctor`'s `mcp proxy` line as showing `not installed` "when no known config has a wrapped entry" — wrong on both counts: `mcpProxyScopes` (`commands/doctor.ts`) prints `not installed (no MCP client config found)` only when NONE of the known client config FILES exists on disk; when at least one exists, it prints one line per existing config with `wrapped N/M stdio servers` even when N is zero (never folded into "not installed" just because nothing is wrapped yet). Reworded both CHANGELOG.md and the spec to match; README.md's installer paragraph never made the wrong claim (it did not mention the not-installed condition at all) and needed no change.
- Task 6 (review, minor): `run-mcp-demo.sh`'s final secret grep never inspected the proxy's own stderr, since `call()` truncates `"$work/err"` on every invocation and only the LAST call's survived to the end. Added an accumulator (`"$work/all-stderr.log"`, appended to after every `call()` and after the new scenario 6's `init` invocation) and included it in the final `absent` checks.
- Task 6 (review, minor): packages/cli/README.md's Commands table described the MCP installer only as `--agent mcp --client <name>`, omitting the `--config <path>` alternative `init-mcp.ts` accepts equally. Added.
- Final review: CRITICAL — the secret-egress guard could be bypassed by padding. Core's candidate extraction scans only the first `MAX_INPUT_CHARS` (262 144) characters of `JSON.stringify(toolInput)`, so a `tools/call` whose serialised arguments ran past that was judged on a prefix of itself: 300 KiB of filler ahead of a `.env` value put that value outside the window and the call was forwarded and allowed (confirmed end-to-end through the real proxy, and reproduced as a failing test before the fix). `judgeToolCall` now measures `JSON.stringify(<the record handed to the engine>)` after the malformed-name check and before `decidePre`, and refuses anything above `MAX_INPUT_CHARS` fail-closed under a new local decision, `mcp-proxy-arguments-too-large`: recorded through `denyDirectly` with a keys-only audit summary (`describeToolInput`, the same reader the Codex/Copilot unreadable-input denies use), answered with the same `isError` tool-result shape as the other denies (`resultType` only for a request that carried `_meta`), and never forwarded. The reason names the window in KiB and never a value. `judge.ts`'s `mcpCallInput` comment, which claimed the guard scans `JSON.stringify(toolInput)` without qualification, now states the window and the refusal.
- Final review: IMPORTANT — a hostile server could hide result content from the scanner by malforming its reply. `mcp-result-text.ts`'s `contentItemText` returned `''` for any non-record item and `itemsText` handed a non-array `content` straight to it, so `{ content: "Ignore all previous instructions…" }` and `{ content: [{ type: "text", text: {…} }] }` both scanned zero characters and never tainted, while a client still rendered every word of them. A bare-string `content` and a bare-string item are now scanned as their own text, and a non-string `text` value — on a content item and on an embedded `resource` alike — is scanned as its JSON, via one shared `textValue` reader; the whole extraction is still clipped to `MCP_MAX_RESULT_CHARS` by `joined`, so the cost is unchanged.
- Final review: IMPORTANT — the "arguments above ~4 MiB are refused fail-closed" claim was wrong: the provenance-atom overflow is content-dependent, and a 5 MiB low-entropy argument was forwarded rather than refused. With the 256 KiB refusal above in place the true statement is that arguments whose serialised form exceeds 256 KiB are refused fail-closed (`mcp-proxy-arguments-too-large`) because the guard scans only that window, and the overflow can no longer be reached through the proxy at all. Reworded in README.md (the limits list and the `tools/call` table row, which claimed the whole argument object is scanned), SECURITY.md's MCP paragraph, CHANGELOG.md's Unreleased entry and spec §2a/§3. `packages/cli/README.md` never repeated the claim and needed no change.
- Final review: minor — the 8 MiB server-line bound was documented as a parse bound without stating its security consequence. README.md's limits, SECURITY.md's scope, CHANGELOG.md and spec §2a/§3 now state plainly that a server result above 8 MiB is forwarded unparsed, unscanned and untainted, so a server can pad a poisoned result past that size to dodge the scan — a documented limit, not a guarantee.
- Final review: minor — the EOF shutdown escalation (client stdin ends → server stdin ends → SIGTERM after the grace → SIGKILL after the grace) had no test; only the signal-relay escalation did. `ignore-sigterm-server.mjs` gained a `setInterval` keep-alive behind `STROQ_MCP_STUB_KEEPALIVE=1` (without it, `stdin.resume()` stops holding its event loop open at EOF and the stub exits by itself, which is why the path was untestable), and `proxy-signals.test.ts` gained a test that ends the client stdin with no signal sent at all and asserts the proxy exits 1 having waited roughly twice the injected `shutdownGraceMs`, bounded at 5 s. Verified as a real test by flipping the flag off: the stub then exits 0 on EOF and the assertion fails.
- Fix-wave re-review: a non-string `text` no longer short-circuits the item's other fields. The fix wave's `contentItemText` read `text` through `textValue` and returned early on any non-empty result, so a junk `text: 0`, `false`, `{}` or `[]` contributed one or two characters and suppressed the item's `uri`/`name`/`description` and an embedded `resource`'s whole body — fields the pre-fix code did scan (measured: `{type:'resource_link', text:0, name:<poison>}` scanned 1 character and did not taint, against 217 characters and a taint before). Only a non-empty STRING `text` now stands in for the rest of the item; a non-string one is read BESIDE `uri`/`name`/`description`, and the same rule governs `resource.text` against `resource.uri`. Covered for each of `0`, `false`, `{}` and `[]` on both item shapes, in extraction (`judge.test.ts`) and against the real engine (`judge-decisions.test.ts`), with a string-`text` control pinning today's behaviour. Two test-hygiene items in the same commit: `proxy-signals.test.ts` gained an `afterEach` that SIGKILLs any child `runMcpProxy` spawned (the keep-alive stub no longer exits on stdin EOF, so an aborted run could orphan it — the handle is captured by a `vi.mock` wrapper that delegates to the real `spawn`, since `runMcpProxy` spawns internally), and `judge.test.ts`'s new describe now saves and restores `process.env['STROQ_HOME']`.

...
- `candidateTokens(toolName, toolInput)` keeps its signature and return type. It now tokenises every window of `textOf(...)` up to `MAX_SCAN_CHARS` — window `i` covers `[i·MAX_INPUT_CHARS − SCAN_OVERLAP, (i+1)·MAX_INPUT_CHARS)` clamped to the text — dedupes across windows (the same `token\nraw` key), applies `MAX_CANDIDATES` PER WINDOW (each window keeps four times the headroom above the densest measured yield, so the guard never decides what is looked up; an earlier window can never starve a later one — the review of Task 1 found that a global cap made ~1.5 MiB of dense padding evict a value inside the bound), and, when the text is longer than `MAX_SCAN_CHARS`, scans exactly the first `MAX_SCAN_CHARS` and ignores the rest (the engine denies the action anyway, see 2b, so nothing past the bound is ever forwarded on trust).

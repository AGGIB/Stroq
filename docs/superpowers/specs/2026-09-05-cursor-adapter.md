...
| `afterFileEdit` | no | `file_path`, `edits[{old_string,new_string}]` | none | classification of the path recorded as `allow(cursor-edit-unenforced)` — audit only; v1 installs on no event that could block an edit (no `beforeFileEdit` exists; the generic `preToolUse` is out of scope, see §3) |
...
- Exit code `2` = block; any other non-zero exit = fail-open unless the hook is configured with `failClosed: true`. Stroq's `init` writes `failClosed: true` on every blocking hook and Stroq additionally prints an explicit `deny` JSON on internal errors for high-impact actions (same as Claude Code). The "a missing Stroq binary blocks instead of silently allowing" guarantee rests entirely on that flag: an uninstalled or unresolvable command makes the shell exit 127, which is simply a non-zero exit, and `failClosed: true` is what turns it into a block rather than an allow.
...
- `packages/cli/src/adapters/cursor.ts`
  - `CursorHookInputSchema` (zod `looseObject`): `conversation_id: string`, `hook_event_name: enum[...]`, `workspace_roots: unknown[]` (default `[]`; the first usable string element is the project), optional `command`, `cwd`, `output`, `stdout`, `stderr`, `exit_code`, `tool_name`, `tool_input: unknown`, `mcp_server_name`, `result_json`, `tool_output`, `file_path`, `content`, `attachments: unknown` (normalised to a list), `edits`. Fields Stroq only reads opportunistically are typed `unknown` on purpose: a shape surprise on a non-blocking event must never throw, because a throw there skips the scan and the taint it would have set.
  - `handleCursorHook(engine, raw): Promise<HookOutput>` — `HookOutput` reused (`stdout`, `exitCode`).
  - Mapping to engine events: `sessionId = conversation_id`; `cwd =` first string `workspace_roots` entry `|| cwd || process.cwd()`; tool names: `Bash` for shell, `mcp__${server}__${tool}` for MCP (server name sanitised to `[A-Za-z0-9_-]`), `Read` for beforeReadFile, `Write` for afterFileEdit; `toolInput`: `{command}` / MCP args (parse `tool_input` string as JSON, fall back to `{raw}`) / `{file_path}`.
...
- `stroq doctor`: a `cursor hooks` line: `project: installed (path)` / `missing`, mirroring the Claude Code line; overall `ok` requires at least one agent installed. An agent that is absent while the other is installed reads `not installed (ok: <other> are)`, so a passing line never shows a green tick next to the word "missing".
- README: "Supported today: Claude Code, Cursor"; Install section gains `--agent cursor`; a "Cursor" subsection listing what is and is not covered (no warning injection after shell output; `beforeReadFile` allow-only; v1 installs on no event that could block an edit, so `config.self` writes through Cursor's editor are audited as `allow(cursor-edit-unenforced)`, not blocked; Cursor's own web reads are not scanned; multi-root workspaces are indexed through the first root; Windows untested).
...
- `stroq attack` scenarios stay Claude-Code-shaped and deliberately include no Cursor scenarios: the engine, rules and policy the suite exercises are shared byte-for-byte between the two adapters, so a Cursor copy of each scenario would re-test the same decision path, and the part that is genuinely Cursor-specific — the wire mapping from hook JSON to engine event and back — is covered by `hook-cursor.e2e.test.ts` and by `run-cursor-demo.sh`, which CI runs.

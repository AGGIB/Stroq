...
What it does **not** detect: a name or an address written in **prose**. That needs NER, and the credible offline option would be this project's first native runtime dependency — the same call that made `stroq run --sandbox` shell out to `srt` rather than link it. Run the MCP demo and the customer record shows both halves: the labelled fields come back as placeholders while `Peter Parker` in the sentence travels untouched; that is the gap, left visible on purpose. [docs/CLOAK-COMPARISON.md](CLOAK-COMPARISON.md) puts this next to AgentCloak Desktop row by row.

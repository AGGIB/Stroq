...
The [Coverage by agent](GUIDE.md#coverage-by-agent) table in the guide is the summary. This file is the full detail behind it: every hook event Stroq installs on, the wire format it reads, and every documented limit — the honest edges, not just the happy path. Each install command below also appears in the README; this is where the "why" for each adapter lives.
...
Stroq installs on seven of Cursor's hook events:

| Cursor event           | What Stroq does                                                                                                                                      | Can it stop the action?                                                                             |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `preToolUse` (`Write`/`Delete` matcher) | Classifies every reported file path before the editor tool runs; refuses unreadable or oversized path lists | Yes — `deny`; `ask` becomes deny because Cursor does not enforce it here |
...
| `afterFileEdit`        | Records a completed edit as a post observation, separately from the `preToolUse` decision | No |

`preToolUse` (matched to `Write` and `Delete`), `beforeShellExecution` and `beforeMCPExecution` are installed with `failClosed: true`, so a crashed or missing Stroq blocks those actions. The four observation/read hooks are installed without it. The `beforeReadFile` refusal of a credential path under taint remains best-effort: an internal error on that event allows the read.

**Limits.**

- **Editor writes are covered only when Cursor emits a matched `Write` or `Delete` tool call.** The generic `preToolUse` gate denies edits to protected config and Git execution files, checking every recognised path field, `file_path` and the camelCase `relativeWorkspacePath`/`targetFile` Cursor uses in its own records included. A write whose path it cannot find is allowed and recorded as `cursor-write-path-unread` rather than denied: Cursor, not the model, chooses those field names, so an unknown shape is a gap in what Stroq knows rather than a way around it, and a deny there would block every file the agent writes. More than 64 paths is still denied. `afterFileEdit` records completed edits as `cursor-edit-observed`, without claiming that its post event enforced the decision. Cursor Tab edits use separate Tab hooks, which Stroq does not install. This integration has been checked with synthetic documented payloads, not a recorded Cursor session.
...
- **Not used:** Cursor's Tab hooks (`beforeTabFileRead`, `afterTabFileEdit`), `postToolUse`, `beforeSubmitPrompt`, `updated_input` rewriting and enterprise/team hook locations. `preToolUse` is matched only to `Write` and `Delete` to avoid duplicate shell/MCP decisions.
...
What it detects: the **exact values** of credentials this machine holds (the same `.env*`, `~/.aws/credentials`, `~/.npmrc`, `~/.netrc`, `~/.docker/config.json` and environment index the egress guard uses — strictly stronger than any pattern), plus `email`, `phone` (E.164's 7–15 digit window), `iban` (ISO 13616 mod-97), `card` (Luhn) and `ssn` (the SSA allocation rules, ITINs included). Person names and street addresses are claimed from the **field** they arrive in rather than from the characters in them: an MCP result is JSON, so `first_name`, `surname`, `attendees`, `street_address` and their spellings label the value more reliably than a model reading it could, at no dependency and for names no English-trained model has seen. A bare `name` and a bare `address` are deliberately excluded — the first is a filename or a branch as often as a person, the second an IP or a wallet — and so are `username` and `login`, which are identifiers the server looks up.

What it does **not** detect: a name or an address written only in **prose**, without a labelled field in the same result. That needs NER, and the credible offline option would be this project's first native runtime dependency — the same call that made `stroq run --sandbox` shell out to `srt` rather than link it. Run the MCP demo and the customer record shows both halves: the labelled fields come back as placeholders while an unlabelled name in a separate result travels untouched; that is the gap, left visible on purpose. [docs/CLOAK-COMPARISON.md](CLOAK-COMPARISON.md) puts this next to AgentCloak Desktop row by row.

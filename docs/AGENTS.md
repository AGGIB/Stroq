...
- **Editor writes are covered only when Cursor emits a matched `Write` or `Delete` tool call.** The generic `preToolUse` gate denies edits to protected config and Git execution files. It checks all recognised path fields and denies a write with no readable path. `afterFileEdit` records completed edits as `cursor-edit-observed`, without claiming that its post event enforced the decision. Cursor Tab edits use separate Tab hooks, which Stroq does not install. This integration has been checked with synthetic documented payloads, not a recorded Cursor session.

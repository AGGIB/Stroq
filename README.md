...
2. **PreToolUse**: `Bash`, `Write`/`Edit`, `Read`, `WebFetch` and `mcp__*` calls are classified into action classes (`shell.network`, `shell.destructive`, `shell.exec_encoded`, `fs.secrets`, `git.push_external`, `config.self`, `config.self_touch`, `mcp.side_effect`, …) and evaluated against the policy. Tainted sessions get `deny` on network/secret/push actions; destructive commands always `ask`; encoded execution and proven self-tampering (writing, editing, or deleting the agent's own security config) are always denied. Commands that only read the security config — `cat`, `grep`, `git status`/`diff`/`add` of it, and the like — are allowed; opening it in an editor asks for confirmation (`config.self_touch`) instead of being denied outright.

...
| `stroq mcp --server <name> --client <client> --cwd <dir> --pass-env <names> -- <cmd…>`                                                                             | The stdio MCP proxy that `init --agent mcp` writes into a client config: judges every `tools/call` against your policy, scans every result on its way back, and starts the server with only the variables `--pass-env` names plus the ones any process needs                                                                      |
